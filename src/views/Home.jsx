...
function Home() {
  ...
  const clients = [
    "sophos.jpg", "sentinelone.jpg", "texub.jpg", "tenable.jpg", "Temp.jpg", "ring.jpg", "proven.jpg", "omnix.jpg", "nvidia.jpg", "nttdata.jpg", "nozomi.jpg", "nexans.jpg", "mimecast.jpg", "ifs.jpg", "cloudbox.jpg", "cisco.jpg", "aveva.jpg", "asus.jpg",
  ];
  const awardItems = [
  ...
  const mottoItems = [
    { title: "People", description: "The heart of our agency, bringing creativity, expertise, and dedication to every project.", icon: <IconFriends stroke={2} />, },
  ...
  const testimonials = [
    { name: "Sarah Johnson", company: "TechNova Inc.", text: "Oak Consulting transformed our brand presence. Their creativity and dedication are unmatched.", image: "testimonial1.jpg", },
    { name: "Ahmed Khan", company: "GlobalTech Solutions", text: "They always helped us tell our story in a way that truly connects with our customers.", image: "testimonial2.jpg", },
    { name: "Lisa Chen", company: "BrightWave Media", text: "A professional, passionate team that always goes the extra mile to deliver impactful campaigns.", image: "testimonial3.jpg", },
  ];
  ...
  const ValueCard = ({ title, description, icon }) => (
    <div className="p-6 h-full flex items-start bg-white rounded-xl border border-gray-200 shadow-sm transition duration-300 hover:shadow-lg text-black max-w-sm">
      ...
      <div>
        <h3 className={`text-xl font-bold mb-4 tracking-wide text-[#FF6600]`}>{title}</h3>
  ...
  return (
    <div id="scroll-container"> 
      ...
      <div ref={sectionRefs.current[0]} className="h-screen relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 z-10">
          <div className="relative text-center text-white px-4">
            ...
            <h1 className="text-4xl md:text-6xl text-[#FF6600] mt-3 font-extrabold tracking-tight">Building Brands</h1>
            <p className="mt-4 text-lg md:text-xl max-w-3xl mx-auto">Turning ideas into stories that resonate, influence, and transform brands</p>
          </div>
        </div>
      </div>

      {/* 2. ABOUT SECTION */}
      <div ref={sectionRefs.current[1]} className="h-screen flex flex-col items-center bg-white p-10">
        <div className="py-10">
          <h2 className="text-5xl font-bold text-center text-black">We Make Things <span className="text-[#FF6600]">Happen</span></h2>
          <div className="flex justify-center mt-3"><div className="h-1 bg-[#ff6600] w-[120px]"></div></div>
          <p className="text-justify px-10 text-xl leading-9 mt-10 max-w-4xl mx-auto">We are a team of creative and strategic thinkers with decades of PR and marketing experience. We don’t just execute campaigns – we craft stories that resonate, strategies that deliver results, and experiences that elevate brands.</p>
          <p className="text-justify px-10 text-xl py-4 leading-9 my-5 max-w-4xl mx-auto">From technology to lifestyle, consumer products to travel, we bring professional expertise and a relentless focus on impact to every project, helping your business stand out in a crowded market.</p>
        </div>
      </div>

      {/* 3. OUR MOTTO SECTION */}
      <div ref={sectionRefs.current[2]} className="h-screen flex flex-col items-center bg-white p-10">
        <div className="py-10">
          <h2 className="md:text-5xl text-3xl font-bold text-center text-black">Our <span className={primaryOrange}>Motto</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto px-4">
            {mottoItems.map((item, index) => (
              <div key={`motto-${index}`} className="p-6 flex flex-col justify-between bg-white rounded-xl border border-gray-200 shadow-sm text-black transition duration-300 hover:shadow-lg">
                <div className="mb-3">
                  <div className="w-10 h-10 flex items-center justify-center rounded-full bg-orange-100 text-[#FF6600] text-2xl ">
                    {item.icon}
                  </div>
                ...
                <div>
                  <h3 className="text-xl font-bold mb-1 tracking-wide text-black">{item.title}</h3>
                  <p className="text-sm text-justify leading-6 text-zinc-800">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div ref={sectionRefs.current[3]} className="h-screen flex flex-col justify-center items-center bg-white p-10">
        <div className="py-2">
          <h2 className={`md:text-5xl text-3xl font-bold text-center text-black`}>What <span className={primaryOrange}>Drives Us</span></h2>
          <div className="flex justify-center mt-2"><div className="h-1 bg-[#ff6600] w-[120px]"></div></div>
          <p className="text-center text-xl text-zinc-800 max-w-3xl mx-auto mt-6 mb-10">We are motivated by our clients success and guided by our core values:</p>
      ...
      <div ref={sectionRefs.current[4]} className="h-screen flex flex-col justify-center items-center bg-white p-10">
        <div className=" py-10">
          <h2 className={`md:text-5xl text-3xl font-bold text-center text-black mb-2`}>Trusted by <span className={primaryOrange}>Industry Leaders</span></h2>
          <div className="flex justify-center mt-3"><div className="h-1 bg-[#ff6600] w-[120px]"></div></div>
          <p className="text-center text-xl mt-6 mb-10">Partnering with world-class brands to deliver exceptional results.</p>
          <div className="relative w-full overflow-hidden">
            <div className="flex animate-scroll space-x-10 mt-25">
              {[...clients, ...clients].map((client, index) => (
                <img key={index} src={`clients/${client}`} alt={client} className="h-16 w-auto object-contain hover:grayscale-100 transition duration-300" />
              ))}
            </div>
          </div>
        </div>
      ...
      <div ref={sectionRefs.current[5]} className="h-screen flex flex-col justify-center items-center bg-white p-4">
        <div className=" py-10">
          <h2 className="md:text-5xl text-3xl font-bold text-center text-black">What Our <span className={primaryOrange}>Clients Say</span></h2>
          <div className="flex justify-center mt-3 mb-10"><div className="h-1 bg-[#ff6600] w-[120px]"></div></div>
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto px-4 mt-10">
            {testimonials.map((t, index) => (
              <div key={index} className="bg-gray-50 rounded-xl border border-gray-100 p-6 transition duration-300 hover:bg-gray-100 text-left">
                <div className="mb-1 text-6xl min-h-12 h-12 leading-none text-[#FF6600] font-serif">&rdquo;</div>
                <p className="text-gray-700 text-justify tracking-tight italic mb-4">"{t.text}"</p>
                <div className="h-px bg-gray-300 w-full mb-3"></div>
                <div><h4 className="font-bold text-md text-black">{t.name}</h4><p className="text-sm text-gray-500">{t.company}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div ref={sectionRefs.current[6]} className="h-screen flex flex-col justify-center items-center bg-white p-5">
        <div className=" py-10">
          <h2 className={`md:text-5xl text-3xl font-bold text-center text-black `}>Awards & <span className={primaryOrange}>Recognition</span></h2>
          <div className="flex justify-center mt-3"><div className="h-1 bg-[#ff6600] w-[120px]"></div></div>
          <div className="max-w-4xl mx-auto space-y-8 mt-10 px-4">
            <div className="grid md:grid-cols-2 gap-6">
              {awardItems.map((award, index) => (
                <div key={index} className="p-4 flex items-center rounded-xl border border-zinc-300 text-black shadow-sm">
                  <div className="mr-4 flex-shrink-0">
                    <div className="w-10 h-10 flex items-center justify-center rounded-full text-black text-2xl ring-2 ring-yellow-500/30 bg-orange-100">{award.icon}</div>
                  </div>
                  <div><h3 className="font-bold text-lg text-black leading-snug">{award.mainTitle}</h3><p className="text-sm text-gray-500 leading-snug">{award.subtitle}</p></div>
                </div>
              ))}
            </div>
            <div className="grid md:grid-cols-2 gap-6 ">
              <div className="w-full h-56 rounded-xl overflow-hidden shadow-2xl">
                <img src="Awards/image-1.jpeg" alt="" className="w-full h-full object-cover"/>
              </div>
              <div className="w-full h-56 rounded-xl overflow-hidden shadow-2xl">
                <img src="Awards/image-2.jpeg" alt="" className="w-full h-full object-cover"/>
              </div>
            </div>
          </div>
        </div>
